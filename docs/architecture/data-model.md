...
| remember_token | varchar(100) | YES | null | Remember Me 機能用トークン（Laravel 標準カラム） |
...
| source | varchar(20) | NO | browser | 入力元: `browser`（ブラウザ録音）/ `plaud`（PLAUD Zapier Webhook） |
| audio_path | varchar(500) | YES | null | ストレージ上の音声ファイルパス（例: voice/{user_id}/{YYYY}/{MM}/{DD}/{uuid}.mp4）。`source=plaud` の場合は null |
| transcribed_text | text | NO | - | 文字起こし結果テキスト。`source=browser` は Whisper API 出力、`source=plaud` は PLAUD 出力 |
| recorded_at | timestamp | NO | - | 録音日時 |
| duration_seconds | int | YES | null | 録音時間（秒）。`source=plaud` では取得できない場合 null |
| created_at | timestamp | NO | now() | 作成日時 |
| updated_at | timestamp | NO | now() | 更新日時 |
...
**整合性備考**:
- `user_id` の冗長保持は意図的（Policy / Gate でのユーザー認可を JOIN なしに行うため）。`daily_log_id` の `user_id` との一致はアプリ層（Action クラス）で必須検証する（`voice_diaries.user_id == daily_logs.user_id`）
- PLAUD Webhook の `title` は MVP では保存しない（`transcribed_text` と `recorded_at` を正とし、`title` は表示・分析の入力に利用しない）
...
- `user_id`（FK用）
- `daily_log_id`（FK用）
- `(user_id, source_type, video_id, occurred_at)` **UNIQUE** — `activity_like` / `liked_video` / `playlist_item` の重複除外用。`video_id` が NOT NULL のシグナルにのみ有効（MySQL は UNIQUE index 中の NULL 値を重複扱いしないため、`subscription` は自動的にこの制約をバイパスする）
- `(user_id, channel_id)` — `subscription` の参照用。重複制御はアプリ層の `firstOrCreate(['user_id', 'source_type' => 'subscription', 'channel_id'])` で行う（MySQL は部分インデックスをサポートしないため、DB 制約での分離は不可）

**FK**: `user_id` → `users(id)` ON DELETE CASCADE, `daily_log_id` → `daily_logs(id)` ON DELETE SET NULL
**整合性備考**:
- `user_id` の冗長保持は意図的（Policy / Gate でのユーザー認可を JOIN なしに行うため）。`daily_log_id` の `user_id` との一致はアプリ層（Action クラス）で保証する
- `video_id` は `youtube_video_catalog.video_id` への論理参照（外部キー制約は張らない — カタログ未取得でも保存できる設計）
...
**Index**:
- `user_id`（FK用）
- `(user_id, analysis_type, period_type, period_start)` **UNIQUE** — 同一ユーザー・同一観点・同一期間のレポートは1件のみ保持。再実行・リトライ時は INSERT ではなく `updateOrCreate` で上書きする（重複による履歴蓄積は行わない）
...
- タイムゾーン方針: `recorded_at` / `occurred_at` / `created_at` / `updated_at` は UTC で保存する。`daily_logs.log_date` はユーザーのローカルタイムゾーン日付で保持し、日次集計は `log_date` を基準に行う
...
| 2026-04-19 | レビュー指摘4点を修正。①remember_token説明修正 ②youtube_signals重複除外をsource_type別に明記（video系: DB unique / subscription: アプリ層guard）③analysis_reportsにuniqueを追加しupsert方針を明記 ④user_id冗長保持の設計意図を明記 | - |
| 2026-04-19 | PLAUD Zapier Webhook対応（F-012）。voice_diaries に source カラム追加、audio_path を nullable に変更 | - |
| 2026-05-17 | Gate 3レビュー指摘を反映。①voice_diaries と daily_logs の所有者一致検証（アプリ層）を明記 ②PLAUD `title` の非保存方針を明記 ③UTC保存 + log_date集計のタイムゾーン方針を明記 | - |
