...
| 目的 | 個人の行動ログ（音声日記・YouTube行動シグナル）を毎日蓄積し、Claude API が5つの観点で定期自動分析するパーソナルツール |
...
| # | 観点 | 説明 | 実行頻度 |
|---|---|---|---|
| 1 | 自動回顧録 | 蓄積ログから振り返りレポートを自動生成 | 週次・月次 |
| 2 | 学習フィルター | 学習傾向・興味分野を抽出・可視化 | 週次・月次 |
| 3 | メンタルトリガー検知 | 気分・感情パターンの検知とアラート | 週次 |
| 4 | AI コーチング | ログをもとにアドバイス・目標提示 | 月次 |
| 5 | ビジネスアイデア提案 | 行動シグナルから推定した興味カテゴリ×日記×学習傾向でアイデア提案 | 月次 |
