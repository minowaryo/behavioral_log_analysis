...
When:  ページをリクエストする
Then:  ログイン画面にリダイレクトされる（Web画面）
  And: 同等のAPIエンドポイントでは HTTP 401 が返却される
```
...
## AC-008: PLAUD 音声メモを Zapier 経由で自動取り込みする（UC-008 / F-012）

### 正常系

```
Given: Zapier が有効な Webhook シークレット付きで transcript と recorded_at を送信する
When:  POST /api/webhooks/zapier/plaud を呼び出す
Then:  VoiceDiary が source: plaud で作成される
  And: 対応する日付の DailyLog が存在しない場合は自動作成される
  And: voice_diary_id が返却される
```

```
Given: recorded_at が欠落している
When:  Webhook を受信する
Then:  受信日時を recorded_at として処理が継続される
  And: VoiceDiary が作成される
```

### 異常系

```
Given: Webhook シークレットが不一致である
When:  POST /api/webhooks/zapier/plaud を呼び出す
Then:  HTTP 401 が返却される
  And: VoiceDiary は作成されない
```

```
Given: transcript が空文字である
When:  POST /api/webhooks/zapier/plaud を呼び出す
Then:  HTTP 400 が返却される
  And: VoiceDiary は作成されない
```
...
### セキュリティ
- [ ] 未認証ユーザーが保護された Web 画面にアクセスした場合はログインページへリダイレクトされる
- [ ] 未認証ユーザーが保護された API にアクセスした場合は HTTP 401 が返却される
