...
**関連要件**: F-004〜F-008, F-009, F-011

#### 基本フロー
1. Laravel スケジューラが隔月1日 AM 7:00 に ScheduledAnalysisJob を実行
2. 対象ユーザーの過去60日分のログを集約
3. Claude API に以下6観点の分析を依頼:
   - 自動回顧録、学習フィルター、メンタルトリガー検知
   - AI コーチング（ai_coaching）
   - タイムカプセル（time_capsule）
...
- タイムカプセルは「2ヶ月後の自分へのメッセージ」として別途保存する（UC-006 で閲覧）
- 分析ログが15日未満の場合はスキップ

---

### UC-006: タイムカプセルを作成・閲覧する
...
#### 基本フロー（閲覧）
1. ユーザーが Analysis 画面の「タイムカプセル」タブを開く
2. 開封可能なタイムカプセルがある場合（作成から60日以上経過）は通知バナーを表示
3. ユーザーがタップすると内容（過去の自分からのメッセージ）が表示される

#### 業務ルール
- 2ヶ月レポート実行時に Claude が自動生成するものと、ユーザーが手動作成するものの2種類がある
- 開封条件: 作成日から60日以上経過していること
- 開封済みタイムカプセルは「過去の記録」として常時閲覧可能
